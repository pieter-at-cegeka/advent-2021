package be.acerta.pieter.advent2021.day16;

import java.util.Collections;
import java.util.List;
import java.util.stream.Stream;

import static java.lang.String.format;
import static java.util.stream.Collectors.toList;

public class Puzzle16 {
    public static void main(String... args) {
        List<PacketParser> packetParsers = Stream.of(
                        "D2FE28",
                        "38006F45291200",
                        "EE00D40C823060",
                        "8A004A801A8002F478",
                        "620080001611562C8802118E34",
                        "C0015000016115A2E0802F182340",
                        "A0016C880162017C3686B18A3D4780",
                        "C200B40A82",
                        "04005AC33890",
                        "880086C3E88112",
                        "CE00C43D881120",
                        "D8005AC2A8F0",
                        "F600BC2D8F",
                        "9C005AC2F8F0",
                        "9C0141080250320F1802104A08",
                        "")
                .map(PacketParser::new)
                .collect(toList());

        packetParsers.forEach(packetParser -> {
            System.out.println(format("PacketParser:\n\thexadecimal: %s\n\tbinary: %s",
                    packetParser.getHexadecimalRepresentation(),
                    packetParser.getBinaryRepresentation()));

            Packet packet = packetParser.parsePacket();

            printPacket(packet, 1);
        });
    }

    private static void printPacket(Packet packet, int printDepth) {
        System.out.println(format("%s%s",
                String.join("", Collections.nCopies(printDepth, "\t")),
                packet));

        packet.getSubPackets().forEach(subPacket -> printPacket(subPacket, printDepth + 1));
    }
}
